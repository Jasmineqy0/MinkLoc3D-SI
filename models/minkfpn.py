...
class MinkFPN(ResNetBase):
    ...
    def forward(self, x, y=None):
        ...
        if y:
            ...
            pos_x = torch.zeros_like(x.F)
            pos_y = torch.zeros_like(y.F)
            pos_x[...] = batch_idx_x[:, None]
            pos_y[...] = batch_idx_y[:, None]
            pos_x.to('cuda')
            pos_y.to('cuda')
            ...
            feats_x , feats_y = x.F, y.F
            feats_x = torch.unsqueeze(feats_x, dim=1)
            feats_y = torch.unsqueeze(feats_y, dim=1)
            
            feats_x, feats_y = self.transformer_encoder(
                feats_x, feats_y,
                src_pos= pos_x if transformer_encoder_has_pos_emb else None,
                tgt_pos= pos_y if transformer_encoder_has_pos_emb else None,
            )
        #################################################
